...
#' Generate snow by Polygons
#'
#' Generates summary of snow values by polygons and year using MODIS
#'
#' @param aoi.abbr area of interest
...
extract.modis.snow.polygons <- function(aoi.abbr, polys, year.vec, write.file=FALSE, write.path){
  ...
  for(y in 1:length(year.vec)){
    ...
    for(m in 1:length(month.vec)){
      ...
      MODIStsp(gui             = FALSE,
               ...
               selprod         = "Snow_Cov_Month_0.05Deg CMG (M*D10CM)",
               bandsel         = "SN_COV_MNT",
               sensor          = "Terra",
               user            = "ryan.miller" ,
    ...
    data.path<-paste0(normalizePath(working.dir.path),"\\temp\\Snow_cov_mnt_005dg_v6\\SN_COV_MNT")
